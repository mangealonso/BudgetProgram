...
test('load data remove data', async ({ page }) => {
    ...
    let LoadedExpensesDataCount = await page.locator('#allExpensesPostsSection li').count();
...
test('load data and assert income for january is correct', async ({ page }) => {
    ...
    await page.selectOption('#yearAndMonthSection select', { label: '2023' });

    //Locate jan and check balance
    let jan = await page.locator('#yearAndMonthSection li').filter({ hasText: 'January:' })
...
test('year automatically added and removed in dropdown', async ({ page }) => {
    ...
    let dropDownCount = await page.locator('#yearAndMonthSection select option').count();
    await expect(dropDownCount).toEqual(3);
     ...
     dropDownCount = await page.locator('#yearAndMonthSection select option').count();
     ...
     await page.locator('li:has-text(" Present From Mom, Gift, 500, 2021-01-22") button').click();
          ...
          //Assert Dropdown count is 3 again (Year, 2023 and 2021)
          dropDownCount = await page.locator('#yearAndMonthSection select option').count();
          await expect(dropDownCount).toEqual(3);
});
